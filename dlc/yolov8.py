...
model_path = "/home/gongye/dlc/gongye.dlc"
...
def postProcess(pres, confThresh, NmsThresh):
    ...
    for pred in pres:
        ...
        if pred_class[class_index] > 0.3:
            box = np.array(
                [round(pred[0] - 0.5 * pred[2]), round(pred[1] - 0.5 * pred[3]), round(pred[2]), round(pred[3])])
...
if __name__ == "__main__":
    ...
    # 2.加载模型
    value = aidlite.ANNModel(model_path, [640 * 640 * 3 * 4], [11 * 8400 * 4], 4, 0)
    print("gpu:", value)
    ...
    for root, dirs, files in os.walk(image_path):
        ...
        for file in files:
            ...
            # 5.获取输出
            pred = aidlite.getOutput_Float32()
            print(pred.shape)
            pred = np.array(pred)
            ...
            ret_img = ret_img[:, :, ::-1]
            num += 1
            image_file_name = "/home/gongye/result/dlc/res" + str(num) + ".jpg"
